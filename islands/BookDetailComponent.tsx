...
export default function BookDetailComponent() {
  ...
  const loadBookDetail = async (token: string, bookId: string) => {
    try {
      ...
      const response = await fetch(`/api/book/info?bookId=${bookId}&token=${token}`);
  ...
  const loadChapters = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/book/chapters?bookId=${bookInfo.value.bookId}&token=${token}`);
  ...
  const downloadChapter = async (chapter) => {
    try {
      const token = localStorage.getItem("weread_token");
      const response = await fetch(`/api/book/content?bookId=${bookInfo.value.bookId}&chapterUid=${chapter.chapterUid}&token=${token}`);
  ...
  const downloadAllChapters = async () => {
    ...
    try {
      const token = localStorage.getItem("weread_token");
      let allContent = `${bookInfo.value.title}\n作者: ${bookInfo.value.author}\n\n`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              <a href="/shelf" className="text-gray-600 hover:text-gray-900">书架</a>
              <button 
                onClick={() => {
                  localStorage.clear();
                  window.location.href = "/login";
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-3 text-gray-600">
                    ...
                    {bookInfo.value.totalWords && (
                      <div className="flex items-center">
                        ...
                        <span>{bookInfo.value.totalWords.toLocaleString()} 字</span>
                      </div>
                    )}
                  ...
                  <div className="mt-6 flex flex-wrap gap-4">
                    ...
                    >
                      {chaptersLoading.value ? "加载中..." : showChapters.value ? "刷新章节" : "查看章节"}
                    </button>
              ...
              <div className="bg-white rounded-lg shadow-lg p-6">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                  ...
                  <div className="flex-1 max-w-md">
                    <input
                      ...
                      onInput={(e) => chapterSearchQuery.value = e.currentTarget.value}
                      placeholder="搜索章节..."
                ...
                {filteredChapters.value.length === 0 ? (
                  <div className="text-center py-8 text-gray-500">
                    {chapterSearchQuery.value ? "没有找到匹配的章节" : "暂无章节"}
                  </div>
                ) : (
