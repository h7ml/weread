...
export default function WeReadStyleReaderComponent() {
  ...
  const resetHideTimer = () => {
    ...
    hideTimer.current = setTimeout(() => {
      if (!showSettings.value && !showChapterList.value && !ttsState.value.isPlaying) {
  ...
  const loadSettings = () => {
    ...
    fontSize.value = parseInt(localStorage.getItem("weread_fontSize") || defaultSettings.fontSize.toString());
    lineHeight.value = parseFloat(localStorage.getItem("weread_lineHeight") || defaultSettings.lineHeight.toString());
    theme.value = localStorage.getItem("weread_theme") || defaultSettings.theme;
    fontFamily.value = localStorage.getItem("weread_fontFamily") || defaultSettings.fontFamily;
    pageWidth.value = localStorage.getItem("weread_pageWidth") || defaultSettings.pageWidth;
  ...
  const saveSettings = () => {
    ...
    localStorage.setItem("weread_ttsSettings", JSON.stringify(ttsSettings.value));
    localStorage.setItem("weread_autoReading", JSON.stringify(autoReading.value));
  ...
  // 实时保存设置的辅助函数
  const updateTTSSettings = (updates: Partial<typeof defaultSettings.ttsSettings>) => {
  ...
  const setDefaultVoiceForEngine = () => {
    ...
    if (currentVoice) {
      ...
      const voiceExists = availableVoices.some(v => (v.short_name || v.name) === currentVoice);
      if (voiceExists) {
        return; // 当前语音仍然有效
      }
    }
    ...
    if (firstVoice) {
      ...
      console.log(`为 ${currentEngine} 引擎设置默认语音:`, firstVoice.display_name || firstVoice.name);
    }
  };
  ...
  const getAvailableVoicesForEngine = (engine: string) => {
    if (engine === "browser") {
      ...
      return ttsState.value.azureVoices.filter((v: any) => v.provider === "leftsite");
    } else if (engine === "openxing") {
      return ttsState.value.azureVoices.filter((v: any) => v.provider === "openxing");
    }
    return [];
  };
  ...
  const switchTTSEngine = (newEngine: string) => {
    ...
    if (firstVoice) {
      ...
      console.log(`切换到 ${newEngine} 引擎，默认语音:`, firstVoice.display_name || firstVoice.name);
    } else {
      updates.voiceURI = "";
  ...
  // TTS相关功能
  const extractTextContent = (element: Element): { sentences: string[], elements: Element[] } => {
    ...
    const walkNode = (node: Node, parentElement?: Element) => {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (text && text.length > 0) {
          // 按句号、问号、感叹号分句
          const sentenceArray = text.split(/[。！？；]\s*/).filter(s => s.trim().length > 0);
  ...
  const speakCurrentSentence = async () => {
    ...
    if (state.currentSentenceIndex >= state.sentences.length) {
      // 章节结束，检查是否自动翻页
      if (ttsSettings.value.autoNext && currentChapterIndex.value < chapters.value.length - 1) {
    ...
    // 根据引擎选择播放方式
    if ((ttsSettings.value.engine === 'leftsite' || ttsSettings.value.engine === 'openxing') && ttsState.value.serviceStatus === 'available') {
  ...
  const speakWithExternalTTS = async (sentence: string) => {
    try {
      ...
      audio.onerror = (error) => {
        console.error(`${ttsSettings.value.engine.toUpperCase()} TTS播放错误:`, error);
      ...
      await audio.play();
      console.log(`${ttsSettings.value.engine.toUpperCase()} TTS播放句子:`, sentence.substring(0, 50) + '...');
      
    } catch (error) {
      console.error(`${ttsSettings.value.engine.toUpperCase()} TTS完全失败，降级到浏览器TTS:`, error);
      await speakWithBrowserTTS(sentence);
    }
  };
  ...
  const highlightCurrentSentence = () => {
    // 移除之前的高亮
    const prevHighlight = contentRef.current?.querySelector('.tts-current-sentence');
    if (prevHighlight) {
      prevHighlight.classList.remove('tts-current-sentence');
    }
    
    // 高亮当前句子
    const currentElement = ttsState.value.sentenceElements[ttsState.value.currentSentenceIndex];
    if (currentElement) {
  ...
  const stopTTS = () => {
    ...
    // 移除所有高亮
    const highlights = contentRef.current?.querySelectorAll('.tts-current-sentence');
  ...
  const startAutoScroll = () => {
    ...
    autoScrollTimer.current = setInterval(() => {
      if (!autoReading.value.isActive || autoReading.value.isPaused || !contentRef.current) {
        if (autoScrollTimer.current) {
          clearInterval(autoScrollTimer.current);
          autoScrollTimer.current = null;
        }
      ...
      if (currentScrollTop + clientHeight >= scrollHeight - 50) {
        if (autoReading.value.autoNext && currentChapterIndex.value < chapters.value.length - 1) {
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${getThemeClasses()}`}>
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-current mx-auto"></div>
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${getThemeClasses()}`}>
      {/* 顶部工具栏 */}
      <div className={`fixed top-0 left-0 right-0 z-50 bg-current/5 backdrop-blur-sm border-b border-current/10 transition-transform duration-300 ${
        ...
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex-1 text-center">
            <h1 className="text-lg font-medium truncate px-4">{chapterTitle.value}</h1>
          ...
          <button
            onClick={() => {showSettings.value = !showSettings.value; resetHideTimer();}}
            className="p-2 rounded-lg hover:bg-current/10 transition-colors"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1 bg-current/20 rounded-full h-3 overflow-hidden relative progress-bar-container">
                <div 
                  ...
                  style={{ 
                    width: `${(ttsState.value.currentSentenceIndex / Math.max(1, ttsState.value.sentences.length)) * 100}%` 
                  }}
                >
                </div>
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-xs font-medium text-blue-900/80 drop-shadow-sm">
                    {Math.round((ttsState.value.currentSentenceIndex / Math.max(1, ttsState.value.sentences.length)) * 100)}%
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col items-end text-right min-w-max">
                <span className="text-sm font-medium opacity-90">
                  {ttsState.value.currentSentenceIndex + 1} / {ttsState.value.sentences.length}
                </span>
          ...
          <div className="px-4 py-2">
            <div className="flex items-center space-x-3">
              <button
                ...
                title={autoReading.value.isActive ? "停止自动滚动" : "开始自动滚动"}
              >
                {autoReading.value.isActive ? (
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zM7 8a1 1 0 012 0v4a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v4a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" />
              ...
              <div className="flex-1 bg-current/20 rounded-full h-3 overflow-hidden relative progress-bar-container">
                <div 
                  ...
                  style={{ 
                    width: contentRef.current ? 
                      `${Math.min(100, (contentRef.current.scrollTop / Math.max(1, contentRef.current.scrollHeight - contentRef.current.clientHeight)) * 100)}%` : 
                      '0%'
                  }}
                >
                </div>
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-xs font-medium text-green-900/80 drop-shadow-sm">
                    {contentRef.current ? 
                      Math.round((contentRef.current.scrollTop / Math.max(1, contentRef.current.scrollHeight - contentRef.current.clientHeight)) * 100) : 
                      0}%
                  </div>
                </div>
              </div>
      ...
      {/* 主要内容区域 */}
      <div className={`${(ttsState.value.isPlaying || autoReading.value.isActive) ? 'pt-32' : 'pt-20'} pb-20`}>
      ...
      {/* 底部工具栏 */}
      <div className={`fixed bottom-0 left-0 right-0 z-50 bg-current/5 backdrop-blur-sm border-t border-current/10 transition-transform duration-300 ${
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${
                autoReading.value.isActive ? 'bg-green-600 text-white' : 'hover:bg-current/10'
              }`}
              title="自动滚动阅读"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              onClick={toggleTTS}
              className={`p-2 rounded-lg transition-colors ${
                ttsState.value.isPlaying ? 'bg-blue-600 text-white' : 'hover:bg-current/10'
              }`}
              title="语音朗读"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072M18.364 5.636a9 9 0 010 12.728m-9.192-9.192L7.05 7.05A7 7 0 105 12a7 7 0 002.05-4.95l2.122-2.122z" />
              </svg>
            </button>
          </div>
              ...
              <div className="space-y-8 animate-fade-in">
                <div className="bg-current/3 rounded-2xl p-6 settings-card">
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {[
                      { key: "light", name: "默认", bg: "bg-white", text: "text-gray-900", ring: "ring-gray-300" },
                      { key: "sepia", name: "护眼", bg: "bg-amber-50", text: "text-amber-900", ring: "ring-amber-300" },
                      { key: "dark", name: "夜间", bg: "bg-gray-800", text: "text-gray-100", ring: "ring-gray-600" },
                      { key: "night", name: "深邃", bg: "bg-gray-900", text: "text-gray-200", ring: "ring-gray-700" },
                      { key: "green", name: "清新", bg: "bg-green-50", text: "text-green-900", ring: "ring-green-300" },
                      { key: "blue", name: "海洋", bg: "bg-blue-50", text: "text-blue-900", ring: "ring-blue-300" },
                      { key: "purple", name: "神秘", bg: "bg-purple-50", text: "text-purple-900", ring: "ring-purple-300" },
                      { key: "pink", name: "温柔", bg: "bg-pink-50", text: "text-pink-900", ring: "ring-pink-300" }
                    ].map((t) => (
                      <button
                        key={t.key}
                  ...
                  <div className="space-y-6">
                    <div>
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex items-center space-x-3">
                          <input
                            ...
                            onChange={(e) => {
                              const value = Math.max(12, Math.min(32, parseInt(e.currentTarget.value) || 12));
                      ...
                      <div 
                        ...
                        style={{ '--slider-value': `${((fontSize.value - 12) / (32 - 12)) * 100}%` } as any}
                      >
                        <input
                          type="range"
                          min="12"
                          ...
                          onInput={(e) => {
                            ...
                            if (parent) {
                              parent.style.setProperty('--slider-value', `${percentage}%`);
                            }
                          }}
                        />
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex items-center space-x-3">
                          <input
                            ...
                            onChange={(e) => {
                              const value = Math.max(1.2, Math.min(2.5, parseFloat(e.currentTarget.value) || 1.2));
                      ...
                      <div 
                        className="range-slider"
                        style={{ '--slider-value': `${((lineHeight.value - 1.2) / (2.5 - 1.2)) * 100}%` } as any}
                      >
                        <input
                          type="range"
                          min="1.2"
                          ...
                          onInput={(e) => {
                            ...
                            updateReadingSettings('lineHeight', value);
                            // 更新滑块填充
                            const percentage = ((value - 1.2) / (2.5 - 1.2)) * 100;
                            const parent = e.currentTarget.parentElement as HTMLElement;
                            if (parent) {
                              parent.style.setProperty('--slider-value', `${percentage}%`);
                            }
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {[
                          ...
                          <button
                            key={width.key}
                            onClick={() => updateReadingSettings('pageWidth', width.key)}
                            className={`p-3 rounded-xl border-2 transition-all btn-scale ${
                          ...
                          >
                            <div className="text-sm font-medium">{width.name}</div>
                            <div className="text-xs opacity-60">{width.desc}</div>
                          </button>
                        ))}
                      </div>
                    </div>
            ...
            {activeSettingsTab.value === "voice" && (
              <div className="space-y-8 animate-fade-in">
                <div className="bg-current/3 rounded-2xl p-6 settings-card">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className={`w-2 h-2 rounded-full ${
                        ttsState.value.serviceStatus === 'available' ? 'bg-green-500' :
                        ttsState.value.serviceStatus === 'checking' ? 'bg-yellow-500 animate-pulse' :
                        'bg-red-500'
                      }`}></div>
                      <span className="text-xs opacity-70">
                        {ttsState.value.serviceStatus === 'available' ? '服务正常' :
                         ttsState.value.serviceStatus === 'checking' ? '检测中...' :
                         '服务异常'}
                      </span>
                    </div>
                  </div>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-sm opacity-70">免费使用，无需配置</div>
                        </div>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-sm opacity-70">
                            {ttsState.value.serviceStatus === 'available' ? '高音质，多语音选择' : '服务不可用'}
                          </div>
                        </div>
                        <div className="text-2xl">🎯</div>
                      </div>
                      {ttsState.value.serviceStatus === 'unavailable' && (
                        <div className="absolute inset-0 bg-gray-500/20 rounded-xl flex items-center justify-center">
                          <span className="text-sm text-red-600 font-medium">服务不可用</span>
                        </div>
                      )}
                    </button>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-sm opacity-70">
                            {ttsState.value.serviceStatus === 'available' ? '超拟人语音体验' : '服务不可用'}
                          </div>
                        </div>
                        <div className="text-2xl">⭐</div>
                      </div>
                  ...
                  {(ttsSettings.value.engine === "leftsite" || ttsSettings.value.engine === "openxing") && ttsState.value.serviceStatus === 'unavailable' && (
                ...
                {(ttsSettings.value.engine === "leftsite" || ttsSettings.value.engine === "openxing") && ttsState.value.serviceStatus === 'available' && (
                  <div className="bg-current/3 rounded-2xl p-6 settings-card">
                    <h3 className="text-lg font-semibold mb-4 flex items-center">
                      <span className="mr-2">🎤</span>
                      语音选择
                    ...
                    <select
                      ...
                      onChange={(e) => updateTTSSettings({ voiceURI: e.currentTarget.value })}
                    ...
                    >
                      ...
                      {ttsState.value.azureVoices
                        ...
                        .map((voice: any) => (
                          <option key={voice.short_name || voice.name} value={voice.short_name || voice.name}>
                            {voice.display_name || voice.displayName || voice.local_name} 
                ...
                <div className="bg-current/3 rounded-2xl p-6 settings-card">
                  ...
                  <div className="space-y-6">
                    <div>
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex items-center space-x-3">
                          <input
                            ...
                            onChange={(e) => {
                              const value = Math.max(0.5, Math.min(2, parseFloat(e.currentTarget.value) || 1));
                      ...
                      <div 
                        className="range-slider"
                        style={{ '--slider-value': `${((ttsSettings.value.rate - 0.5) / (2 - 0.5)) * 100}%` } as any}
                      >
                        <input
                          type="range"
                          min="0.5"
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex items-center space-x-3">
                          <input
                            ...
                            onChange={(e) => {
                              const value = Math.max(10, Math.min(100, parseInt(e.currentTarget.value) || 80));
                      ...
                      <div 
                        className="range-slider"
                        style={{ '--slider-value': `${((ttsSettings.value.volume - 0.1) / (1 - 0.1)) * 100}%` } as any}
                      >
                        <input
                          type="range"
                          min="0.1"
                    ...
                    <div className="flex items-center justify-between p-4 bg-current/5 rounded-xl">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm opacity-70">章节读完自动跳转</div>
                        </div>
                      </div>
                      <input
                        type="checkbox"
                ...
                <div className="bg-current/3 rounded-2xl p-6 settings-card">
                  ...
                  <div className="space-y-6">
                    <div>
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex items-center space-x-3">
                          <input
                            ...
                            onChange={(e) => {
                              const value = Math.max(10, Math.min(100, parseInt(e.currentTarget.value) || 50));
                              autoReading.value = {
                                ...autoReading.value,
                                scrollSpeed: value
                              };
                      ...
                      <div 
                        className="range-slider"
                        style={{ '--slider-value': `${((autoReading.value.scrollSpeed - 10) / (100 - 10)) * 100}%` } as any}
                      >
                        <input
                          type="range"
                          min="10"
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-4 bg-current/5 rounded-xl">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm opacity-70">启用丝滑的滚动效果</div>
                          </div>
                        </div>
                        <input
                          type="checkbox"
                      ...
                      <div className="flex items-center justify-between p-4 bg-current/5 rounded-xl">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <div className="font-medium">自动翻页</div>
                            <div className="text-sm opacity-70">滚动到底部自动下一章</div>
                          </div>
                        </div>
                        <input
                          type="checkbox"
      ...
      {showChapterList.value && (
        <div className="fixed inset-0 z-50 bg-black/50" onClick={() => showChapterList.value = false}>
          <div 
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (index !== currentChapterIndex.value) {
                      await navigateToChapter(index > currentChapterIndex.value ? "next" : "prev");
                    }
                  }}
