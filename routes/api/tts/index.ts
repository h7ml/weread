...
async function handleLeftsiteTTS(
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Leftsite TTS服务响应错误:", response.status, response.statusText);
      return new Response(JSON.stringify({
...
// 处理 OpenXing TTS
async function handleOpenXingTTS(text: string, voice: string): Promise<Response> {
  console.log("使用 OpenXing TTS 服务，语音:", voice, "文本:", text.substring(0, 50) + "...");
  ...
  try {
    ...
    if (!response.ok) {
      console.error("OpenXing TTS 服务响应错误:", response.status, response.statusText);
      return new Response(JSON.stringify({
    ...
    if (result.results && result.results.length > 0) {
      ...
      if (audioResult.status === "completed" && audioResult.filename) {
        // 构建音频文件URL
        const audioUrl = `https://tts.openxing.top/api/audio/${encodeURIComponent(audioResult.filename)}`;
