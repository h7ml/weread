...
export async function handler(req: Request) {
  ...
  try {
    ...
    for await (const entry of kv.list({ prefix: ["user"] })) {
      logger.debug("Checking user entry:", entry.key, "skey:", entry.value.skey?.substring(0, 8) + "...");
    ...
    if (!userInfo) {
      ...
      for await (const entry of kv.list({ prefix: ["user"] })) {
        logger.info("- User:", entry.value.vid, "skey:", entry.value.skey?.substring(0, 8) + "...");
      }
    ...
    logger.info(`Successfully fetched ${total} books (showing ${paginatedBooks.length} on page ${page}/${totalPages}) for user ${userInfo.vid}`);
