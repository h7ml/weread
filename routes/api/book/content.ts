import { getBookInfo, getChapterContent } from "@/apis";
...
export async function handler(req: Request) {
  ...
  try {
    ...
    logger.info(
      `Fetching chapter content for bookId: ${bookId}, chapterUid: ${chapterUid}`,
    );
    ...
    try {
      ...
      logger.warn(
        "Failed to get book format, using default epub:",
        error.message,
      );
    }
    ...
    logger.info(
      `Successfully fetched chapter content for: ${chapterUid} (${content.length} chars)`,
    );
