import { HttpClient, buildQueryString, sign, calcHash, currentTime, chk, dH, dS, dT } from "@/utils";
...
export async function getBookInfo(bookId: string, cookie?: string): Promise<Book> {
  try {
    ...
    if (cookie) {
      ...
      const response = await client.get<any>(`/web/book/info?bookId=${bookId}`, { headers });
  ...
  try {
    if (format === "epub") {
      ...
      const [e0, e1, e2, e3] = await Promise.all([
        getChapterFragment("/web/book/chapter/e_0", bookId, chapterUid, 0, cookie),
        getChapterFragment("/web/book/chapter/e_1", bookId, chapterUid, 0, cookie),
        getChapterFragment("/web/book/chapter/e_2", bookId, chapterUid, 1, cookie),
        getChapterFragment("/web/book/chapter/e_3", bookId, chapterUid, 0, cookie)
      ]);
      ...
      if (e0 && e1 && e3) {
        ...
        // 移除base64占位图
        processedHtml = processedHtml.replace(/(<img[^>]+?)(src="data:[^"]+")/g, "$1");
        
        // 将data-src替换为src
        processedHtml = processedHtml.replace(/(<img[^>]+?)data-src="/g, '$1src="');
      ...
      const [t0, t1] = await Promise.all([
        getChapterFragment("/web/book/chapter/t_0", bookId, chapterUid, 0, cookie),
        getChapterFragment("/web/book/chapter/t_1", bookId, chapterUid, 1, cookie)
      ]);
