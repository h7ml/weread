 ...
 * 获取单个任务
 */
export async function getTask(vid: number, taskId: string): Promise<ReadTask | null> {
  const kv = await getKv();
 ...
 * 更新任务最后阅读时间
 */
export async function updateTaskReadTime(vid: number, taskId: string): Promise<void> {
 ...
 * 切换任务状态
 */
export async function toggleTaskStatus(vid: number, taskId: string): Promise<boolean> {
  const task = await getTask(vid, taskId);
  if (task) {
